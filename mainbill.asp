...
<%
'USER CHECK
If Cint(Request.Cookies("LBUSERTYPE")) = 0 Then
...
'	If rsConfirm("showinst") = True Then tmpVis2 = "Checked"
	...
	If rsConfirm("happen") = 1 then happen1 = "SELECTED"
	If rsConfirm("happen") = 2 then happen2 = "SELECTED"
	If rsConfirm("noreas") = 1 Then noreasq1 = "SELECTED"
	If rsConfirm("noreas") = 2 Then noreasq2 = "SELECTED"
	If rsConfirm("noreas") = 3 Then noreasq3 = "SELECTED"
	If rsConfirm("noreas") = 7 Then noreasq7 = "SELECTED"
	If rsConfirm("noreas") = 8 Then noreasq8 = "SELECTED"
	DSnoreas = Z_FixNull(rsConfirm("DSnoreas"))
	tmpFoncall = ""
...
If Not rsIntr.EOF Then
	...
	If rsIntr("certIntr") Then ttrate = 38.00
Else
...
<html>
	<head>
		...
		{ 
	      ...
	      // results.
		  	...
		  	if (decMile > <%=tmpmilecap%>) //interpreter
		  	{
				...
				{
					document.frmConfirm.txtDecTT.value = <%=Z_CZero(tmpDecTT)%>;
		  		document.frmConfirm.txtDecMile.value = <%=Z_CZero(tmpDecMile)%>;
	  ...
	  <% End If %>
		...
		function SaveBill(xxx, yyy)
		{
			
			if (document.frmConfirm.chkBilTInst.checked == true) {
				if (yyy != 273) {
					var traveldollar = document.frmConfirm.txtBilTInst.value;
					var travelrate = document.frmConfirm.txtTTRate.value;
					var doverrate = Math.round(traveldollar / travelrate * 100) / 100;
					var newdollaramount = Math.round(doverrate * travelrate * 100) / 100;
					...
					var mylang = <%=tmpLang%>;
					if (mylang != 52 && mylang != 78 && mylang != 81 && mylang != 90) {
						newdollaramount = 95;
					}
					else {
						var traveldollar = document.frmConfirm.txtBilTInst.value;
			...
			<% If tmpIntr <> 0 Then %>
				if (document.frmConfirm.radioStat[4].checked == true) {
					if (document.frmConfirm.chkBillIntr.checked == true) {
						if (document.frmConfirm.chkBilTIntr.checked == false && document.frmConfirm.chkBilMIntr.checked == false) {
							alert("This is a canceled-billable appointment. If you wish to pay for Travel Time and/or Mileage, please check the overwrite checkbox and enter the amount.");
							return;
						}
					}
					else {
						if (document.frmConfirm.chkBilTIntr.checked == true || document.frmConfirm.chkBilMIntr.checked == true) {
							alert("This is a canceled-billable appointment. If you wish to pay for Travel Time and/or Mileage, please check the 'Pay Interpreter' checkbox.");
							return;
						}
					}
				}
		...
		function InstRates()
		{
			...
			{
				document.frmConfirm.txtTTRate.value = <%=ttrate%> ;
				document.frmConfirm.txtMRate.value = 0.50;
		...
		{
			<% If OverTTInst = "" Then %>
				if (document.frmConfirm.chkBillInst.checked == true) {
					...
					document.frmConfirm.txtBilTInst.value = 0;
				}
			<% Else %>
			...
			<% If OverMInst = "" Then %>
				if (document.frmConfirm.chkBillInst.checked == true) {
					...
					document.frmConfirm.txtBilMInst.value = 0;
				}
			<% Else %>
		...
		function chkans() {
			document.frmConfirm.DSnoreas.readOnly = true;
			document.frmConfirm.DSnoreas.value = '';
			if (document.frmConfirm.selHap.value == 0) { 
			document.frmConfirm.noreasq.disabled = true;
			document.frmConfirm.noreasq.value = 0;
			document.frmConfirm.DSnoreas.readOnly = true;
			document.frmConfirm.DSnoreas.value = '';
			}
			else if (document.frmConfirm.selHap.value == 1) { 
			document.frmConfirm.noreasq.disabled = false;
			document.frmConfirm.DSnoreas.readOnly = true;
			}
			else if (document.frmConfirm.selHap.value == 2) { 
			document.frmConfirm.noreasq.disabled = true;
			document.frmConfirm.noreasq.value = 0;
			document.frmConfirm.DSnoreas.readOnly = true;
			document.frmConfirm.DSnoreas.value = '';
		...
		function chkans2() {
			if (document.frmConfirm.noreasq.value == 0) { 
			document.frmConfirm.DSnoreas.readOnly = true;
			document.frmConfirm.DSnoreas.value = '';
			}
			else if (document.frmConfirm.noreasq.value == 1) { 
			alert("If the reminder call was completed, please input the date it was completed.");
			document.frmConfirm.DSnoreas.readOnly = false;
			}
			else if (document.frmConfirm.noreasq.value == 2) { 
			document.frmConfirm.DSnoreas.value = '';
			}
			else if (document.frmConfirm.noreasq1.value == 3) { 
			//alert("Please contact Language Bank ASAP.");
			document.frmConfirm.DSnoreas.value = '';
			}
		}
		-->
		...
		<body onload='CancelMe(); CancelReason(<%=tmpCancel%>); CompleteMe();MissedMe(); MissedReason(<%=tmpMiss%>);InstRates(); myfee(); chkans();
			<% If tmpIntr > 0 Then %>
				PayMe(); initMap();'
			<% Else %>
				noMap();'
							...
							<table cellSpacing='2' cellPadding='0' width="100%" border='0'>
								...
								<tr><td>&nbsp;</td></tr>
									...
									<!--	<tr>
										...
										<tr>
											<td align='right'>Appointment Happened?</td>
											<td>
												<select name='selHap' class='seltxt' style='width:50px;' onchange="chkans();">
													<option value='0'>&nbsp;</option>
													<option value='1' <%=happen1%>>NO</option>
													<option value='2' <%=happen2%>>YES</option>
												</select>
												&nbsp;&nbsp;Reason:
												<select name='noreasq' class='seltxt' style='width:150px;' onchange="chkans2();">
													<option value='0'>&nbsp;</option>
													<option value='1' <%=noreasq1%>>Client did not show for the appointment</option>
													<option value='2' <%=noreasq2%>>Provider canceled appointment when interpreter arrived</option>
													<option value='3' <%=noreasq3%>>Provider had me interpret for another client not listed on vform</option>
													<option value='7' <%=noreasq7%>>Patient refused interpreter service</option>
													<option value='8' <%=noreasq8%>>No phone number provider on vform</option>
												</select>
												&nbsp;&nbsp;Date:
												<input class='main' size='11' maxlength='10' name='DSnoreas' value='<%=DSnoreas%>' onKeyUp="javascript:return maskMe(this.value,this,'2,5','/');">
											</td>
										</tr>
										<tr>
								...
								<tr>
									...
									<tr>
										<td colspan='10' align='center' height='100px' valign='bottom'>
												...
												<input class='btn' type='button' value='Save' <%=billedna%> onmouseover="this.className='hovbtn'" onmouseout="this.className='btn'" onclick='SaveBill(<%=Request("ID")%>, <%=tmpInst%>);'>
