...
ElseIf tmpReport(0) = 3 Then
	...
	CSVHead = "Request ID,Institution, Department, Appointment Date, Client Last Name, Client First Name, Language, Interpreter Last Name, Interpreter First Name, Appointment Start Time, " & _
		"Appointment End Time, Hours, Rate, Travel Time, Mileage, Emergency Surcharge, Total, Comments, DOB, Customer ID, DHHS, Requesting Person, User"
	
	sqlRep = "SELECT claimant, judge, meridian, nhhealth, wellsense, DocNum, billingTrail, ReqID, HPID, syscom, processed, hasmed, outpatient, medicaid, vermed, autoacc, wcomp, drg, pid, request_T.[index] as myindex, status, [Last Name], [First Name], Clname, Cfname, AStarttime, AEndtime, " & _
		"Billable, DOB, emerFEE, class, TT_Inst, M_Inst, request_T.InstID as myinstID, DeptID, LangID, appDate, InstRate, bilComment, custID, ccode, billgroup, IntrID, appTimeFrom, appTimeTo, distcode " & _
		...
		"(Status = 1 OR Status = 4) AND Processed IS NULL AND ProcessedMedicaid IS NULL"
	...
	If Not rsRep.EOF Then 
		...
		Do Until rsRep.EOF
			...
			Else
				deptdrg = True
			End If
			PayMed = PaytoMedicaid(rsRep("outpatient"), rsRep("hasmed"), rsRep("vermed"), rsRep("autoacc"), rsRep("wcomp"), deptdrg, rsRep("IntrID"), rsRep("medicaid"), rsRep("meridian"), rsRep("nhhealth"), rsRep("wellsense"))
      ...
      '      If tmpMin > 0.75 And tmpMin <= 0.99 Then billhours = Int(billhours) + 1
			'		End If
			'	End If
			'	If Z_Czero(Instr(rsRep("syscom"), "hours billed to institution.")) = 0 Then rsRep("syscom") = rsRep("syscom") & "<br>" & billhours & " hours billed to institution. 4 hours billed to medicaid/MCO"
			...
			If IncludeReq Then
				...
				CSVBodyLine = CB & rsRep("myindex") & "," & GetInst2(rsRep("myinstID")) & "," &  Replace(GetMyDept(rsRep("DeptID")), " - ", "") & "," & rsRep("appDate") & "," & rsRep("Clname") & "," & rsRep("Cfname") &  "," & GetLang(rsRep("LangID")) & "," & rsRep("Last Name") & _
				...
				If rsRep("myinstID") <> 108 And rsRep("myinstID") <> 30 Then 'exclude DHHS and LSS
					...
					If rsRep("class") <> 5 Then
						...
						ElseIf rsRep("myinstID") =  41 Or rsRep("myinstID") =  15 Or rsRep("myinstID") =  70 Or rsRep("DeptID") = 645 Or rsRep("myinstID") =  33 Then 'concord hosp, fam and mchc and com council nashua, riverbend
							CSVBodyBill = CSVBodyBill & """" & "DOTC" & """,""" & _
						...
						ElseIf rsRep("myinstID") = 860 Then 'umass med
							CSVBodyBill = CSVBodyBill & """" & "DOTC" & """,""" & _
								"0" & """,""" & tmpccode & """,""" & rsRep("appDate") & " " & rsRep("Cfname") & " " & rsRep("Clname") & " - " & _
...
ElseIf tmpReport(0) = 16 Then 'billing w/o tagging
	...
	sqlRep = "SELECT claimant, judge, request_T.meridian AS meri, request_T.nhhealth AS NHH, request_T.wellsense AS WS, Docnum, billingTrail, ReqID, HPID, syscom, hasmed, outpatient, request_T.medicaid AS medi, vermed, autoacc, wcomp, drg, pid, request_T.[index] as myindex, status, [Last Name], [First Name], Clname, Cfname, AStarttime, AEndtime, " & _
		"Billable, DOB, emerFEE, class, TT_Inst, M_Inst, request_T.InstID as myinstID, DeptID, LangID, appDate, InstRate, bilComment, custID, ccode, billgroup, IntrID, appTimeFrom, appTimeTo, distcode " & _ 
		"FROM request_T, interpreter_T , dept_T WHERE request_T.[instID] <> 479 AND request_T.deptID =  dept_T.[index] AND IntrID = interpreter_T.[index]  AND " & _
		"(Status = 1 OR Status = 4) AND Processed IS NULL AND ProcessedMedicaid IS NULL"
	...
	If Not rsRep.EOF Then 
		...
		Do Until rsRep.EOF
			...
			Else
				deptdrg = True
			End If
			PayMed = PaytoMedicaid(rsRep("outpatient"), rsRep("hasmed"), rsRep("vermed"), rsRep("autoacc"), rsRep("wcomp"), deptdrg, rsRep("IntrID"), rsRep("medi"), rsRep("meri"), rsRep("NHH"), rsRep("WS"))
			'If PayMed And rsRep("vermed") = 0 Then IncludeReq = False
				...
				If rsRep("myinstID") <> 108 And rsRep("myinstID") <> 30 Then 'exclude DHHS and LSS
					...
					If rsRep("emerFEE") = True Then 
						If rsRep("class") <> 5 Then
							...
							Else 
								If rsRep("class") = 1 Or rsRep("class") = 2 Or rsRep("class") = 4 Then
									If rsRep("custID") = "UMass Community Serv" Then
										CSVBodyBill = CSVBodyBill & """" & "DOTC" & """,""" & _
											"0" & """,""" & "LB Emer Fee" & """,""" & rsRep("appDate") & " " & rsRep("Cfname") & " " & rsRep("Clname") & " - Emer Fee" & """,""" & date & """,""" & _
											rsRep("distcode") & """,""" & "1" & """" & vbCrLf
									Else
...
ElseIf tmpReport(0) = 39 Then 'medicaid sim
	...
	CSVHead = "Request ID,Institution, Department, Appointment Date, Client Last Name, Client First Name, Medicaid,MCO, Language, Interpreter Last Name, Interpreter First Name, Appointment Start Time, " & _
	...
	sqlRep = "SELECT syscom, wid, medicaid, meridian, nhhealth, wellsense, vermed, autoacc, wcomp, drg, pid, request_T.[index] as myindex, status, [Last Name], [First Name], Clname, Cfname, AStarttime, AEndtime, " & _
		...
		"AND (Status = 1 OR Status = 4) AND ProcessedMedicaid IS NULL AND Processed IS NULL"
	...
	If Not rsRep.EOF Then 
		...
		Do Until rsRep.EOF
			...
			strBody = strBody & "<tr bgcolor='" & kulay & "' onclick='PassMe(" & rsRep("myindex") & ")'>" & _
				...
				"<td class='tblgrn2'><nobr>" & hmo & "</td>" & vbCrLf & _
				"<td class='tblgrn2'><nobr>" & GetLang(rsRep("LangID")) & "</td>" & vbCrLf & _
				"<td class='tblgrn2'><nobr>" & strIntrName & "</td>" & vbCrLf & _
				"<td class='tblgrn2'><nobr>" & strATime & "</td>" & vbCrLf & _
				"<td class='tblgrn2'><nobr>" & BillHours & "</td>" & vbCrLf
			...
			If BillHours >= 2 Then
				...
				'	CSVBodyBill = CSVBodyBill & """" & hmo & """,""" & rsRep("wid") & """,""" & mycode & """,""" & "LBHR" & """,""" & "" & """,""" & _
				...
				'End If
				Else
				'	CSVBodyBillMer = CSVBodyBillMer & """" & hmo & """,""" & rsRep("wid") & """,""" & Progcode(hmoused) & """,""" & "LBHR" & """,""" & "" & """,""" & _
				'		FixDateFormat(rsRep("appDate")) & """,""" & FixDateFormat(rsRep("appDate")) & """,""" & "1" & """,""" & """" & vbCrLf 
				'	tmpLbhrs = BillHours - 2
...
ElseIf tmpReport(0) = 40 Then 'medicaid billing
	RepCSV =  "ALLBillReq" & tmpdate & "-" & tmpTime & ".csv" 
	RepCSVBill = "MedicaidBillReqNew" & tmpdate & "-" & tmpTime & ".csv" 
	RepCSVBillMeri = "MeridianBillReqNew" & tmpdate & "-" & tmpTime & ".csv" 
	RepCSVBillLB = "LBBill" & tmpdate & "-" & tmpTime & ".csv"
	RepCSVBillMHP = "MHPBill" & tmpdate & "-" & tmpTime & ".csv"
	RepCSVBillNHHF = "NHHFBill" & tmpdate & "-" & tmpTime & ".csv"
	RepCSVBillWSHP = "WSHPBill" & tmpdate & "-" & tmpTime & ".csv"
	CSVHeadBill = """" & FixDateFormat(date) & "LB Billing" & """"
	Set rsRep = Server.CreateObject("ADODB.RecordSet")
	strHead = "<td class='tblgrn'>Request ID</td>" & vbCrlf & _ 
		"<td class='tblgrn'>Institution</td>" & vbCrlf & _
	...
	CSVHead = "Request ID,Institution, Department, Appointment Date, Client Last Name, Client First Name, Medicaid,MCO, Language, Interpreter Last Name, Interpreter First Name, Appointment Start Time, " & _
		"Appointment End Time, Hours, Rate, Travel Time, Mileage, Emergency Surcharge, Total, Comments, DOB, DHHS"	
	' add vermed = 1 AND if medicaid billing is go 
	sqlRep = "SELECT billingTrail, syscom ,wid, medicaid, meridian, nhhealth, wellsense, vermed, autoacc, wcomp, drg, pid, request_T.[index] as myindex, status, [Last Name], [First Name], Clname, Cfname, AStarttime, AEndtime, " & _
	...
	If Not rsRep.EOF Then 
		...
		Do Until rsRep.EOF
			...
			If BillHours >= 2 Then
				'If BillHours = 2 Then
				If hmoused <> 1 Then 'not meridian
				'	CSVBodyBill = CSVBodyBill & """" & hmo & """,""" & rsRep("wid") & """,""" & Progcode(hmoused) & """,""" & "LBHR" & """,""" & "" & """,""" & _
				'		FixDateFormat(rsRep("appDate")) & """,""" & FixDateFormat(rsRep("appDate")) & """,""" & "1" & """,""" & """" & vbCrLf 
				'Else
				'	CSVBodyBill = CSVBodyBill & """" & rsRep("medicaid") & """,""" & rsRep("wid") & """,""" & Progcode(rsRep("wid")) & """,""" & "LBHR" & """,""" & "" & """,""" & _
				'		FixDateFormat(rsRep("appDate")) & """,""" & FixDateFormat(rsRep("appDate")) & """,""" & "1" & """,""" & """" & vbCrLf 
...
ElseIf tmpReport(0) = 57 Then 'billables MA 
	...
	sqlInst = "SELECT DISTINCT(request_T.[instID]) AS myINST, Facility FROM request_T, institution_T, dept_T " & _
		...
		"UPPER(Bstate) = 'MA' AND [appdate] <= '" & tmpReport(2) & "' AND [appdate] >= '" & tmpReport(1) & "' " & _
...
ElseIf tmpReport(0) = 60 Then 'did not happen 
	...
	Else
		...
		CSVBody = "< --- No records found --- >"
	End If	
ElseIf tmpReport(0) = 61 Then 'Total hours report	montly
...
ElseIf tmpReport(0) = 67 Then 'No Total hours report
	...
	Do Until rsIntrL.EOF
		Set rsRep = Server.CreateObject("ADODB.RecordSet")
		sqlRep = "SELECT intrID FROM request_T WHERE STATUS <> 2 AND STATUS <> 3 AND showintr = 1 AND intrid = " & rsIntrL("intrID") & " "
		If tmpReport(1) <> "" Then
			sqlRep = sqlRep & "AND appDate >= '" & tmpReport(1) & "' "
		End If
		If tmpReport(2) <> "" Then
...
ElseIf tmpReport(0) = 71 Then 'New Total Hours
	...
	strHead = "<td class='tblgrn'>Interpreter</td>" & vbCrlf & _
		...
		"<td class='tblgrn'>Over Time Hours</td>" & vbCrlf & _
		"<td class='tblgrn'>Back Hours</td>" & vbCrlf 
	CSVHead = "Interpreter Last Name, Interpreter First Name,Company Code,Charge Date,File #,Temp Dept,Temp Rate,Regular Hours,OT Hours,Regular Backup Pay Code,Regular Backup Pay Hours,OT Back Pay Code,OT Back Pay Hours,Earnings Code,Amount"
	Set rsRep = Server.CreateObject("ADODB.RecordSet")
	sqlRep = "SELECT * FROM request_T, interpreter_T WHERE intrID = interpreter_T.[index] AND (IntrID <> 0 OR intrID = -1) AND STATUS <> 2 AND STATUS <> 3 AND showintr = 1 "
	If tmpReport(1) <> "" Then
		sqlRep = sqlRep & "AND appDate >= '" & tmpReport(1) & "' "
	...
	If Not rsRep.EOF Then 
		...
		Do Until y = x
			...
			If TotHours > 0 Or myBhrs > 0 Then
				...
				End If
				If myhrsHP > 0 Then
					CSVBody = CSVBody & GetIntr(tmpIntr(y)) & ",ACS," & tmpReport(2) & "," & GetFileNum(tmpIntr(y)) & ",," & Z_GetHigherPay(0, tmpIntr(y)) & "," & Z_FormatNumber(myhrsHP,2) & vbCrLf
				End If
				If 	myHHrs > 0 Then
					'IntrRate = Z_GetDefRate(tmpIntr(y)) * 1.5
					'CSVBody = CSVBody & "ACS," & tmpReport(2) & "," & GetIntr(tmpIntr(y)) & "," & GetFileNum(tmpIntr(y)) & ",," & Z_FormatNumber(IntrRate,2) & ",0,0" & ",HWK," & Z_FormatNumber(myHHrs,2) & vbCrLf
				End If
				If myTrain > 0 Then
					cleanOTHrs = myOTHrs
					If myOTHrs = 0 Then cleanOTHrs = "" 	
